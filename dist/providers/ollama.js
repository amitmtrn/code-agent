...
class OllamaProvider {
    constructor(host) {
        ...
        this.primaryUrl = host ? (0, config_1.normalizeUrl)(host) : config_1.config.OLLAMA_BASE_URL;
    ...
    async createClientWithFallback() {
        const urls = (0, config_1.getOllamaFallbackUrls)(this.primaryUrl);
        for (const url of urls) {
            try {
                const testClient = new ollama_1.Ollama({ host: url });
                ...
                if (this.currentUrl !== url) {
                    console.log(chalk_1.default.yellow(`⚠️  Falling back to Ollama server at ${url}`));
                    this.currentUrl = url;
                }
    ...
    async ensureModelExists(model) {
        ...
        try {
            ...
            if (isConnectionError) {
                ...
                console.error(chalk_1.default.red(`   Connection timeout or server unreachable.`));
                console.error(chalk_1.default.yellow(`   Please ensure Ollama is running and accessible at the configured URL.`));
                console.error(chalk_1.default.yellow(`   Continuing without model verification - chat requests may also fail.`));
        ...
        try {
            ...
            if (isConnectionError) {
                ...
                try {
                    ...
                    console.log(chalk_1.default.yellow(`🔄 Retrying chat request with fallback client...`));
                    ...
                    console.error(chalk_1.default.red(`   ${causeInfo}`));
                    console.error(chalk_1.default.yellow(`   Server: ${this.currentUrl}`));
                    console.error(chalk_1.default.yellow(`   Please check that Ollama is running and accessible.`));
                        ...
                        if (toolCallMatch) {
                            try {
                                ...
                                if ((parsed.cmd && Array.isArray(parsed.cmd)) ||
                                    ...
                                    console.log(chalk_1.default.gray(`📝 Extracted valid tool call from error: ${extractedToolCall}`));
                                }
                            }
                            catch (parseError) {
                                // Invalid JSON, don't use it
                                console.log(chalk_1.default.gray('📝 Found JSON-like content in error but failed validation, skipping extraction'));
                            }
                        }
                    }
                }
                try {
                    ...
                    if (extractedToolCall) {
                        console.log(chalk_1.default.blue('🔄 Attempting to use extracted tool call from original error...'));
                        return {
                            message: {
                                role: 'assistant',
                    ...
                    console.log(chalk_1.default.yellow('⚠️ Using safe fallback response due to persistent errors'));
                    return {
                        message: {
                            role: 'assistant',
